...
%%%%%%%%%%% define collector and emitter voltages
%kV_emitter_vals = [50,60,70,80];
kV_emitter_vals = 80;
kV_collector_vals = 0;
%%%%%%%%%%%
...
files = dir('/Users/Devansh/OneDrive - Imperial College London/MIT/UROP/Electro-Static-Solver/Version 14/results_new/results_with_mu_2R4');
...
for j=1:length(kV_collector_vals)
    for k=1:length(geometry_vals)
        for i=1:length(kV_emitter_vals)
            ...
            %try
                full_results = solver_given_rho0(rho0, geometry, constants, plot_debug);
                filename = sprintf('/Users/Devansh/OneDrive - Imperial College London/MIT/UROP/Electro-Static-Solver/Version 14/results_new/solution_%ikVe_%icm.mat',kV(1),round(geometry.d*100));
